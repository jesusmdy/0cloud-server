...
class Folder:
  ...
  def list_folders(parent_id: str = None, user_id: str = None) -> list:
      ...
      for folder_id in folders:
          folder = Folder.get_direct_folder(folder_id)
          ...
          if folder['parent_id'] == parent_id:
              children_folders.append(folder_id)
      return [
          Folder.get_direct_folder(folder_id) for folder_id in children_folders
      ]

  def list_all_folders(user_id: str = None) -> list:
      """List all folders with optional parent filtering"""
      folders = Database.keys(f"user:{user_id}:folders:*")
      return [Folder.get_direct_folder(folder_id) for folder_id in folders]